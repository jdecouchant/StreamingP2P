...
public class Server {
    ...
    // Membership
    private Set<Short> epochAbsentNodes; // Nodes that left the session since
    // the beginning of new epoch
    ...
    // Constructor
    Server(int listeningPort, ArrayList<Node> nodesList, int FANOUT,
            int UPDATES_NBR_PER_ROUND, int updatesLen, int EPOCH) {
        this.selfId = 0;
        this.roundId = 1;
        ...
        for (int i = 0; i < NB_SENDER; ++i) {
            ...
            senders[i] = new Sender(0, outMessages.get(i), updatesLen,
                    nodesList);
    ...
    public void newRound() {
        ...
        if (epochOrFirstRound) {
            ...
            epochListPlus1 = new HashSet<Short>(); // epochListPlus1 = epochList
            // U
            ...
            if (DEBUG) {
                ...
                System.out.println("Epoch list + 1: "
                        + epochListPlus1.toString());
                System.out.println("New nodes + 2: "
                        + epochListPlus2.toString());
            }
        }
        ...
        Short epochAbsentNodesArray[] = epochAbsentNodes
                .toArray(new Short[epochAbsentNodes.size()]);
        Short epochListPlus1Array[] = epochListPlus1
                .toArray(new Short[epochListPlus1.size()]);
        ...
        if (availableClients.size() > 0) {
            ...
            for (int firstUpdateId = 0; firstUpdateId < UPDATES_NBR_PER_ROUND; firstUpdateId += UPDATES_NBR_PER_SERVE) {
                ...
                for (int clientIndex = 0; clientIndex < Math.min(FANOUT,
                    ...
                    if (epochOrFirstRound
                            && !nodesThatReceivedEpochListPlus1
                            .contains(toClientId)) {
                        if (DEBUG)
                            System.out.println("\tSend epochList+1 to node "
                                    + toClientId);
    ...
    public void receiveJOIN_MESSAGE(Message message) {
        ...
        if (DEBUG)
            System.out.println("\tNode " + message.senderId
                    + " is waiting to join the session");
    ...
    public void sendJOIN_MESSAGE_RESP(short toPartnerId) {
        ...
        Message sendMessage = new Message(roundId,
                MessageType.JOIN_SESSION_RESP, selfId, toPartnerId);
    ...
    public void receivePOM_MESSAGE(Message message) { // Currently, an incorrect
        // node is removed from
        // all lists, and added
        // to the list of absent
        // nodes
        if (DEBUG)
            System.out.println("\n\nNode " + message.senderId
                    + " is incorrect!\n\n");
        ...
        Message sendMessage = new Message(roundId,
                MessageType.EVICTION_NOTIFICATION, selfId, incorrectNodeId);
